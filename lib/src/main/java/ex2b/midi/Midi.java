...
public class Midi {
    ...
    public final static javax.sound.midi.Instrument DEFAULT_INSTRUMENT = Instrument.PIANO;
    
    // active MIDI channels, assigned to instruments
    private final Map<ex2b.midi.Instrument, MidiChannel> channels = new HashMap<ex2b.midi.Instrument,MidiChannel>();
     ...
     */
    public void play(int note, int duration, ex2b.midi.Instrument instr) {
     ...
     * @param instr: instrument in midi.Instrument enum.
     */
    public void beginNote(int note, ex2b.midi.Instrument instr) {
        MidiChannel channel = getChannel(instr);
        synchronized (channel) {
            channel.noteOn(note, VELOCITY);
        }
     ...
     * @param instr: instrument in midi.Instrument enum.
     */
    public void endNote(int note, ex2b.midi.Instrument instr) {
        MidiChannel channel = getChannel(instr);
        synchronized (channel) {
     ...
     */
    private void log(int note, ex2b.midi.Instrument instr, boolean isNoteOn) {
     ...
     * modifies this
     */
    private MidiChannel getChannel(ex2b.midi.Instrument instr) {
     ...
     * @param instr
     */
    private void patchInstrumentIntoChannel(MidiChannel channel, ex2b.midi.Instrument instr) {
